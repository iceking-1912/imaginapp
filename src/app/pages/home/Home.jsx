...
const RenderCards = ({ data, title }) => {
  {
    if (data.length > 0) {
      ...
      return (
        <h2 className="font-bold text-[#150dff] text-xl uppercase">{title}</h2>
      );
    }
  }
};
const Home = () => {
  ...
  const ranimgfetch = async (preimg, allPosts) => {
    try {
      ...
      const ranChoice = Math.floor(
        Math.random() * Object.keys(allPosts).length
      );
          ...
          <>
            {searchText && (
              <h2 className="font-medium text-black text-xl mb-3">
                Showing Results for{" "}
                <span className="text-[#150dff] font-extrabold ">
                  {searchText}
                </span>
              </h2>
            )}
